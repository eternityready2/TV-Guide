...
def get_data(the_date):
    ...
    try:
        ...
        response = requests.get( 
            f"https://api.zype.com/program_guides/66ad433a8b13f6001fb82b24/entries?api_key=GiD2nuFEeFeJgx5y-TOhYaglOM5-UsgqLlX8tMzHpx8w1mEtKRtVDWaWp_ez6pdX&disable_pagination=true",
            headers=headers
        )
    ...
    for item in response.json()['response']:
        ...
        this_date = this_date.astimezone(central_tz)

        if this_date.date() == the_date:
            programs.append(
                {
                    "starts": this_date.strftime("%H%M"),
