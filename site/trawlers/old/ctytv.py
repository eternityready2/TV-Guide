    ...
    for row in rows:
        ptime = datetime.datetime.combine(
            date, datetime.datetime.strptime(
                row.findAll('th')[0].text, "%I:%M %p").time())
        if ptime.hour < 0:
            continue
        ...
        days_programs.append({"date": date.strftime(
            "%Y-%m-%d"), "starts": time_str, "starts_datetime": ptime, "program_name": desc})
