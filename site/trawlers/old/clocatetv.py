from .common import Trawler, correct_timezone_offsets
...
def get_locatetv_date(the_date, offset, network_name):
    ...
    for item in items:
        ...
        desc = item.find("ul").findAll("li")[1].find(
            "div").find("a").text.replace("&#039;", "'")

        try:
            ptime = datetime.datetime.combine(
                the_date, datetime.datetime.strptime(
                    prog_time, "%I:%M%p").time())
        except Exception:
            continue
            ...
            ptime = datetime.datetime.combine(
                the_date, datetime.datetime.now().time()) - datetime.timedelta(minutes=mins)
        ...
        programs.append({"date": the_date.strftime("%Y-%m-%d"),
                         "starts": time_str,
...
class TrawlerLocateTV(Trawler):
    ...
    def get_info_for_days(days, network_name="smile-of-a-child"):
        ...
        for d in range(7):
            ...
            schedule.update(
                {day.strftime("%Y-%m-%d"): get_locatetv_date(day, d, network_name)})

        correct_timezone_offsets(schedule)
        # print schedule
        return schedule
