...
def get_word_week(day):
    ...
    try:
        # print timestamp
        response = urllib.request.urlopen(URL + date_string)
    ...
    for day in days:
        ...
        prog_date = datetime.datetime.strptime(
            date_heading, "%B %d, %Y").date()
        ...
        for row in rows:
            ...
            days_programs.append({"date": prog_date.strftime(
                "%Y-%m-%d"), "starts": time_str, "starts_datetime": ptime, "program_name": link})
