...
def get_3abn_date(the_date, abn_network_id):
    ...
    try:
        data = requests.get(URL + encoded).json()
    except Exception():
        return []
    ...
    for program in data["programs"]:
        p_datetime = datetime.datetime.strptime(
            program["UserFullDateTime"].replace(
                "Z", ""), "%Y-%m-%dT%H:%M:%S")
        # print p_datetime
        # print program["Series"] + "," + program["UTCDateTime"] + "," +
        # program["Duration"]
        ...
        program = {
            "program_name": program["Series"],
            "episode_name": program["Content"],
            "date": p_datetime.strftime("%Y-%m-%d"),
            "starts": p_datetime.strftime("%H%M"),
            "duration": minutes}
...
class Trawler3ABN(Trawler):
    ...
    def get_info_for_days(days, abn_network_id=1):
        ...
        for day in days:
            ...
            schedule.update(
                {day.strftime("%Y-%m-%d"): get_3abn_date(day, abn_network_id)})
        return schedule
