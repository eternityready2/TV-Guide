...
def get_data(the_date):
    try:
        response = requests.get(
            "http://llbnlatino.hostedbroadcasting.com/external_schedule/select_calendar_view_day/1?day={}&month={}&year={}".format(the_date.day, the_date.month, the_date.year),
    ...
    html = match.group(1).encode('latin1', errors='backslashescape').decode('unicode_escape').replace('<\\/t', '</t')
...
class Trawlerllbnlatino(Trawler):
    @staticmethod
